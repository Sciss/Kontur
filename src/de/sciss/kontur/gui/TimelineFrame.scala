...
import de.sciss.tint.sc.{ ServerOptions }
   ...
   private class ActionBounce extends MenuAction {
      ...
      def perform( tracks: List[ Track ], span: Span, descr: AudioFileDescr ) {
         try {
            ...
            so.outputBusChannels.value    = 2   // XXX
            ...
            val audioPrefs                = app.getUserPrefs().node( PrefsUtil.NODE_AUDIO )
            ...
            context.perform {
               ...
               scTL     = new SCTimeline( scDoc, timelineView.timeline )
            ...
            BasicWindowHandler.showErrorDialog( frame.getWindow, e, getValue( Action.NAME ).toString )}
      }
