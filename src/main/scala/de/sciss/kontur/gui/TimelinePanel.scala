...
final class TimelinePanel(/* val tracksView: TracksView,*/ val timelineView: TimelineView)
  ...
  import TimelinePanel._
	...
	private var vpSelections        = List.empty[ViewportSelection]
  ...
  private def updatePlayHead(pos: Long, oldShown: Boolean): Unit = {
    ...
    if (oldShown) {
      if (math.abs(x - oldX) > 50) {
        ...
        val minX = math.min(x, oldX)
        val maxX = math.max(x, oldX)
  ...
  protected def updatePositionAndRepaint(): Unit = {
		...
		if( pEmpty ) {
			if( cEmpty ) return
			val x   = math.max( 0, vpPositionRect.x )
			val x2  = math.min( vpRecentRect.width, vpPositionRect.x + vpPositionRect.width )
