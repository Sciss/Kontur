...
object StakeBorderViewMode {
  ...
  case object None extends StakeBorderViewMode { final val id = 0 }
  /** Rounded box for stake borders */
  case object Box extends StakeBorderViewMode { final val id = 1 }
  /** Rounded box with region name for stake borders */
  case object TitledBox extends StakeBorderViewMode { final val id = 2 }

  def apply(id: Int): StakeBorderViewMode = (id: @switch) match {
    case None     .id => None
    case Box      .id => Box
...
abstract class BasicTrackStakeTool[P](protected val trackList: TrackList, protected val timelineView: TimelineView)
  ...
  protected def dragCancel(d: this.Drag): Unit = dispatch(DragCancel)
...
class TrackMuteTool(protected val trackList: TrackList, protected val timelineView: TimelineView)
  extends TrackStakeTool {
...
class TrackAuditionTool(doc: Session, protected val trackList: TrackList, protected val timelineView: TimelineView)
  extends TrackStakeTool {
